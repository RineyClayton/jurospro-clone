    ...
    if Client.query.count() == 0:
        ...
        for i in range(3):
            inst = Installment(
                ...
                amount=round(l.total_amount() / 3, 2)
            )
            db.session.add(inst)
        db.session.commit()

    return 'DB pronta — admin/admin123'
...
def export_csv():
    ...
    for it in Installment.query.join(Loan).join(Client).add_columns(
        Client.name, Loan.id, Installment.number, Installment.due_date,
        Installment.amount, Installment.paid, Installment.paid_at
    ).all():
